...
faz.bot({pattern: ['update'], fromMe: true, dontAddCommandList: true, desc: "to update the bot remotly"},(async (message, client) => {
    await git.fetch();
...
faz.bot({pattern: ['update now'], fromMe: true,dontAddCommandList: true, desc: "updating"},  (async (message, client) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
